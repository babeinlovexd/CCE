...
class TabAstronomy(ctk.CTkFrame):
    def __init__(self, master, project, **kwargs):
        ...
        # Suns
        self.suns_frame = ctk.CTkFrame(self)
        self.suns_frame.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)

        ctk.CTkLabel(self.suns_frame, text="Suns", font=("Arial", 14, "bold")).pack(pady=5)
        self.suns_list = ctk.CTkScrollableFrame(self.suns_frame)
        self.suns_list.pack(fill="both", expand=True, padx=5, pady=5)
        self.btn_add_sun = ctk.CTkButton(self.suns_frame, text="Add Sun", command=self.add_sun_row)
        self.btn_add_sun.pack(pady=5)

        # Moons
        self.moons_frame = ctk.CTkFrame(self)
        self.moons_frame.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)

        ctk.CTkLabel(self.moons_frame, text="Moons", font=("Arial", 14, "bold")).pack(pady=5)
        self.moons_list = ctk.CTkScrollableFrame(self.moons_frame)
        self.moons_list.pack(fill="both", expand=True, padx=5, pady=5)
        self.btn_add_moon = ctk.CTkButton(self.moons_frame, text="Add Moon", command=self.add_moon_row)
        self.btn_add_moon.pack(pady=5)
    ...
    def add_sun_row(self, sun=None):
        ...
        dawn_ent = ctk.CTkEntry(row_frame, placeholder_text="Dawn (tick)", width=70)
        dawn_ent.pack(side="left", padx=2)

        zenith_ent = ctk.CTkEntry(row_frame, placeholder_text="Zenith", width=70)
        zenith_ent.pack(side="left", padx=2)

        dusk_ent = ctk.CTkEntry(row_frame, placeholder_text="Dusk", width=70)
    ...
    def add_moon_row(self, moon=None):
        ...
        cycle_ent = ctk.CTkEntry(row_frame, placeholder_text="Cycle (days)", width=80)
        cycle_ent.pack(side="left", padx=5)

        offset_ent = ctk.CTkEntry(row_frame, placeholder_text="Offset", width=50)
