...
class TabHolidays(ctk.CTkFrame):
    def __init__(self, master, project, **kwargs):
        ...
        # Holidays
        self.holidays_frame = ctk.CTkFrame(self, corner_radius=10)
        self.holidays_frame.grid(row=0, column=0, sticky="nsew", padx=(20, 10), pady=20)

        ctk.CTkLabel(self.holidays_frame, text="🎉 " + _("Holidays"), font=("Arial", 16, "bold")).pack(pady=(15, 5))
        self.holidays_list = ctk.CTkScrollableFrame(self.holidays_frame, height=300)
        self.holidays_list.pack(fill="both", expand=True, padx=15, pady=5)
        self.btn_add_holiday = ctk.CTkButton(self.holidays_frame, text=_("Add Holiday"), command=self.add_holiday_row)
        self.btn_add_holiday.pack(pady=(10, 15))

        # Leap Rules
        self.leap_frame = ctk.CTkFrame(self, corner_radius=10)
        self.leap_frame.grid(row=0, column=1, sticky="nsew", padx=(10, 20), pady=20)

        ctk.CTkLabel(self.leap_frame, text="➕ " + _("Leap Rules"), font=("Arial", 16, "bold")).pack(pady=(15, 5))
        self.leap_list = ctk.CTkScrollableFrame(self.leap_frame, height=300)
        self.leap_list.pack(fill="both", expand=True, padx=15, pady=5)
        self.btn_add_leap = ctk.CTkButton(self.leap_frame, text=_("Add Leap Rule"), command=self.add_leap_row)
        self.btn_add_leap.pack(pady=(10, 15))
    ...
    def add_holiday_row(self, holiday=None):
        ...
        month_vals = [_("(None - Abs Day)")] + [m.name for m in self.project.calendar_model.months]
        month_menu = ctk.CTkOptionMenu(row_frame, values=month_vals, width=120)
        month_menu.pack(side="left", padx=2)

        day_ent = ctk.CTkEntry(row_frame, placeholder_text=_("Day Num"), width=70)
        day_ent.pack(side="left", padx=2)

        wd_var = ctk.BooleanVar()
        wd_check = ctk.CTkCheckBox(row_frame, text=_("Is Weekday"), variable=wd_var, width=50)
        wd_check.pack(side="left", padx=5)
        Tooltip(wd_check, _("If unchecked, this holiday pauses the normal rotation of weekdays (it exists 'outside' the week)."))
    ...
    def add_leap_row(self, rule=None):
        ...
        desc_ent = ctk.CTkEntry(row_frame, placeholder_text=_("Description"), width=90)
        desc_ent.pack(side="left", padx=2)

        freq_ent = ctk.CTkEntry(row_frame, placeholder_text=_("Every X Years"), width=80)
        freq_ent.pack(side="left", padx=2)

        days_ent = ctk.CTkEntry(row_frame, placeholder_text=_("Days to add"), width=70)
        days_ent.pack(side="left", padx=2)

        month_vals = [_("(None)")] + [m.name for m in self.project.calendar_model.months]
        month_menu = ctk.CTkOptionMenu(row_frame, values=month_vals, width=120)
        month_menu.pack(side="left", padx=2)
