...
class ViewerView(ctk.CTkFrame):
    def __init__(self, master, app, project, **kwargs):
        ...
        self.btn_back = ctk.CTkButton(self.header_frame, text="🔙 Editor", width=60, command=self.app.switch_to_editor)
        ...
        self.btn_prev_year = ctk.CTkButton(nav_frame, text="<< Year", width=60, command=self.prev_year)
        self.btn_prev_year.pack(side="left", padx=2)

        self.btn_prev_month = ctk.CTkButton(nav_frame, text="< Month", width=60, command=self.prev_month)
        self.btn_prev_month.pack(side="left", padx=2)

        self.date_label = ctk.CTkLabel(nav_frame, text=self.get_date_label_text(), font=("Arial", 14, "bold"), width=150)
        self.date_label.pack(side="left", padx=10)

        self.btn_next_month = ctk.CTkButton(nav_frame, text="Month >", width=60, command=self.next_month)
        self.btn_next_month.pack(side="left", padx=2)

        self.btn_next_year = ctk.CTkButton(nav_frame, text="Year >>", width=60, command=self.next_year)
        self.btn_next_year.pack(side="left", padx=2)

        self.btn_all_year = ctk.CTkButton(nav_frame, text="Show All Year", width=80, command=self.show_all_year)
        self.btn_all_year.pack(side="left", padx=5)

        self.jump_entry = ctk.CTkEntry(nav_frame, placeholder_text="Year", width=50)
        self.jump_entry.pack(side="left", padx=2)

        self.btn_jump = ctk.CTkButton(nav_frame, text="Jump", width=40, command=self.jump_year)
        ...
        self.search_panel.pack(side="right", padx=10, pady=10)
        ...
        self.sync_label = ctk.CTkLabel(self.sync_frame, text="Select a day", font=("Arial", 10), text_color="gray")
    ...
    def get_date_label_text(self):
        ...
        return f"Year {self.current_year} - {m_text}"
    ...
    def update_sync_display(self):
        if self.current_selected_day_start_tick is None:
            self.sync_label.configure(text="Select a day")
            return
        ...
        if sec and sec.day_length_ticks > 0:
            ...
            self.sync_label.configure(text=f"Yr {sec_year}, Day {sec_day_in_year} | Time: {sec_time_str}")
        else:
